...
def test_symmetry():
    ...
    #redundants first
    test_subject_projs_red = [np.array([[ 0.56086644,  0.43061449, -0.70710678],
                           [ 0.56086644,  0.43061449,  0.70710678],
                           [-0.60898085,  0.79318492,  0.        ]])
    ,np.array([[-0.14852746,  0.55791839,  0.3291588 , -0.24149772], 
    ...
    #start of customs
    test_subject_projs_custom = [np.array([[ 0.56086644,  0.43061449, -0.70710678],
               [ 0.56086644,  0.43061449,  0.70710678],
               [-0.60898085,  0.79318492,  0.        ]]),
    np.array([[-0.14852746,  0.55791839,  0.3291588 , -0.24149772],  
           [-0.14852746,  0.55791839,  0.3291588 , -0.24149772],
    ...
    for c, coord in enumerate(["redundant", "Custom"]):
        ...
        for t, test in enumerate(test_subjects):
            ...
            symm_obj.run()
            s_vec = SVectors(
                zmat, options, zmat.variable_dictionary_init
            )
            s_vec.run(zmat.cartesians_init,True, second_order=options.second_order)
            symm_obj.make_proj(s_vec)
            assert np.allclose(symm_obj.salc_proj, test_subject_projs[t], rtol=0, atol=1e-8)
